...
class _LeaveScreenState extends ConsumerState<LeaveScreen> {
  ...
  List<OutworkRequest> _outworkRecords = [];
  double _remaining = 0;
  double _total = 0;
  bool _loading = true;
  String? _error;
  int _activeTabIndex = 0; // 0: 휴가, 1: 외근/출장
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final outworkFuture = ref.read(apiClientProvider).get<Map<String, dynamic>>(
        '/api/outwork',
        queryParameters: {'employmentId': emp.id},
      );

      final results = await Future.wait([leaveFuture, outworkFuture]);
      ...
      final outworkList = (outworkRes['records'] as List<dynamic>? ?? [])
          .map((e) => OutworkRequest.fromJson(e as Map<String, dynamic>))
          .toList()
        ..sort((a, b) => b.date.compareTo(a.date));

      final used = leaveList
  ...
  Future<void> _cancelOutwork(String outworkId) async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
    ...
    try {
      await ref.read(apiClientProvider).delete<dynamic>('/api/outwork/$outworkId');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
  ...
  Widget _buildOutworkTab(Map<OutworkStatus, ({Color bg, Color color, String label})> statusStyle, Map<OutworkType, String> typeLabel) {
    return Column(
      children: [
        const Align(
          alignment: Alignment.centerLeft,
          child: Text('외근/출장 신청 내역', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
        ),
        const SizedBox(height: 10),
        if (_outworkRecords.isEmpty)
          Container(
            width: double.infinity,
            padding: const EdgeInsets.symmetric(vertical: 32, horizontal: 16),
            decoration: BoxDecoration(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                const Text(
                  '우측 상단의 신청하기 버튼을 통해 외근/출장을 신청할 수 있습니다.',
                  style: TextStyle(
                    fontSize: 11,
                    fontWeight: FontWeight.w500,
                    color: AppColors.textMuted,
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 6),
                      Text(formattedDate, style: const TextStyle(fontSize: 13, color: AppColors.textSecondary, fontWeight: FontWeight.w600)),
                    ],
                  ),
                  ...
                  if (record.status == OutworkStatus.pending) ...[
                    ...
                    Align(
                      alignment: Alignment.centerRight,
                      child: GestureDetector(
                        onTap: () => _cancelOutwork(record.id),
                        child: Container(
                          padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                          decoration: BoxDecoration(
                            color: AppColors.dangerLight,
